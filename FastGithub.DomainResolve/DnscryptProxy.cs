...
namespace FastGithub.DomainResolve
{
  ...
  sealed class DnscryptProxy
  {
    ...
    public DnscryptProxy(ILogger<DnscryptProxy> logger)
    {
      ...
      this.exeFilePath = Path.Combine(AppContext.BaseDirectory, PATH, OperatingSystem.IsWindows() ? $"{NAME}.exe" : NAME);
      this.tomlFilePath = Path.Combine(AppContext.BaseDirectory, PATH, $"{NAME}.toml");
    }
    ...
    {
      this.TryEnsureExecutablePermission();

      return Process.Start(new ProcessStartInfo
      {
        FileName = this.exeFilePath,
        WorkingDirectory = Path.GetDirectoryName(this.exeFilePath) ?? string.Empty,
    ...
    /// <summary>
    /// 确保 Unix 平台下 dnscrypt-proxy 可执行
    /// </summary>
    private void TryEnsureExecutablePermission()
    {
      ...
      try
      {
        if (File.Exists(this.exeFilePath))
        {
          ...
          {
            FileName = "/bin/chmod",
            ArgumentList = { "+x", this.exeFilePath },
            UseShellExecute = false,
      ...
      {
        this.logger.LogWarning($"设置 dnscrypt-proxy 可执行权限失败：{ex.Message}");
      }
    }
